# Storee Product Knowledge Base

## How to Update Products

Products are stored as **static data** in the frontend at:
- **File**: `frontend/src/data/products.ts`
- **To Update**: Edit the file directly and refresh the browser
- **No backend needed** for product browsing (orders/payments still use backend)

## Brand Overview

**Storee** - *Home to your belongings*

Storee is a premium e-commerce platform specializing in high-quality pouches, bags, and organizers for kids and families. We offer a curated collection of storage solutions designed to keep your belongings organized, protected, and easily accessible. From travel essentials to daily routines, Storee helps parents keep their kids' lives beautifully organized.

### Product Summary
- **Total Products**: 12 unique product lines
- **Products with Images**: 9 (ready for sale)
- **Coming Soon**: 3 products (Crossbody Bag, Medicine Kit, Pencil Pouch)
- **Total Image Variants**: 46 different designs/colors
- **Categories**: Travel kits, pouches, packing organizers, specialty kits, and bags
- **Price Range**: ₹450 - ₹2600

## Quick Price Reference

| Product | Price | Variants | Status |
|---------|-------|----------|--------|
| Pack A Week Kit | ₹2600 | 5 | ✅ Available |
| Packing Cubes | ₹2500 | 5 | ✅ Available |
| Toiletry Kit | ₹1000 | 5 | ✅ Available |
| Shoe Pouch | ₹1000 | 5 | ✅ Available |
| Multipurpose Pouch | ₹900 | 10 | ✅ Available |
| On-the-Go Foldable Pouch | ₹900 | 1 | ✅ Available |
| Foldable Travel Kit | ₹500 | 5 | ✅ Available |
| Accessory Pouch | ₹450 | 5 | ✅ Available |
| Dental Kit | TBD | 5 | ✅ Available |
| Crossbody Bag | TBD | 0 | 🚀 Coming Soon |
| Medicine Kit | TBD | 0 | 🚀 Coming Soon |
| Pencil Pouch | TBD | 0 | 🚀 Coming Soon |

## Product Catalog

### 1. Accessory Pouch (Accessories Kit)
- **Price**: ₹450
- **Variants**: 5 different designs/colors
- **Image Location**: `/images/products/accessories_kit/`
- **Files**: `accessories_kit_1.jpg` to `accessories_kit_5.jpg`
- **Description**: A smart organiser for small accessories like hair clips, rubber bands, and tiny essentials. Designed with thoughtful compartments to keep everything neatly sorted and easy to find. Perfect for everyday use and travel.

### 2. Toiletry Kit
- **Price**: ₹1000
- **Variants**: 5 different designs/colors
- **Image Location**: `/images/products/toiletry_kit/`
- **Files**: `toiletry_kit_1.jpg` to `toiletry_kit_5.jpg`
- **Description**: Keep kids' toiletries organised with this compact and travel-friendly pouch. Designed with a clear front for quick visibility, it's perfect for storing creams, hygiene products, and daily essentials. Lightweight, practical, and easy to clean.

### 3. Foldable Travel Kit
- **Price**: ₹500
- **Variants**: 5 different designs/colors
- **Image Location**: `/images/products/foldable_travel_kit/`
- **Files**: `foldable_travel_kit_1.jpg` to `foldable_travel_kit_5.jpg`
- **Description**: A compact foldable travel organiser designed to keep kids' essentials neatly sorted. With multiple compartments and a space-saving design, it's perfect for travel, sleepovers, and everyday organisation. Lightweight, practical, and easy to carry.

### 4. Pack A Week Kit
- **Price**: ₹2600
- **Variants**: 5 different designs/colors
- **Image Location**: `/images/products/pack_a_week_kit/`
- **Files**: `pack_a_week_kit_1.jpg` to `pack_a_week_kit_5.jpg`
- **Description**: Plan outfits and essentials effortlessly with our Pack A Week Kit. Seven separate pouches help organise each day of the week, all stored inside one larger pouch for easy packing. Ideal for travel, school trips, and organised routines.

### 5. Packing Cubes
- **Price**: ₹2500
- **Variants**: 5 different designs/colors
- **Image Location**: `/images/products/packing_cubes/`
- **Files**: `packing_cubes_1.jpg` to `packing_cubes_5.jpg`
- **Description**: A set of four packing cubes designed to organise clothes, accessories, and essentials inside your suitcase. These lightweight travel organisers maximise space while keeping everything neat and easy to find. Perfect for family travel and kids' packing.

### 6. Shoe Pouch
- **Price**: ₹1000
- **Variants**: 5 different designs/colors
- **Image Location**: `/images/products/shoe_pouch/`
- **Files**: `shoe_pouch_1.jpg` to `shoe_pouch_5.jpg`
- **Description**: A protective shoe pouch designed to keep footwear separate from clothes while travelling. Spacious, easy to pack, and perfect for keeping luggage clean and organised. A must-have travel organiser for kids and families.

### 7. Multipurpose Pouch
- **Price**: ₹900
- **Variants**: 10 different designs/colors
- **Image Location**: `/images/products/multipurpose_pouch/`
- **Files**: `1.jpg` to `5.jpg` (JPG format) and `6.png` to `10.png` (PNG format)
- **Description**: A versatile pouch set designed to organise everything from toys and stationery to travel essentials. With multiple sizes and spacious compartments, it keeps everyday items sorted and easy to access. A practical organiser for home and travel.

### 8. On-the-Go Foldable Pouch
- **Price**: ₹900
- **Variants**: 1 design
- **Image Location**: `/images/products/on_the_go_foldable_pouch/`
- **Files**: `on_the_go_foldable_pouch_1.jpg`
- **Description**: A smart foldable pouch that opens into organised compartments for quick access to essentials. Perfect for travel, school bags, or day trips when you need everything in one place. Compact when folded, spacious when opened.

### 9. Dental Kit (Dental Pouch)
- **Price**: ₹0 (TBD)
- **Variants**: 5 different designs/colors
- **Image Location**: `/images/products/dental_pouch/`
- **Files**: `1.png` to `5.png`
- **Description**: A compact dental organiser designed to keep kids' oral care essentials in one place. Perfect for storing toothbrush, toothpaste, and hygiene products during travel or daily routines. Practical, organised, and easy to carry.

---

## Additional Products (Images Pending)

### 10. Crossbody Bag
- **Price**: ₹0 (TBD)
- **Status**: Description available, product images pending
- **Description**: A lightweight crossbody bag designed especially for kids' little adventures. Spacious enough for small essentials while staying comfortable and easy to carry. Available in fun prints kids will love.

### 11. Medicine Kit
- **Price**: ₹0 (TBD)
- **Status**: Description available, product images pending
- **Description**: A travel-friendly medicine organiser designed to keep your child's medicines neatly arranged. Dedicated loops hold bottles securely to prevent spills, with enough space for strips, syrups, and essentials. Perfect for travel and everyday preparedness.

### 12. Pencil Pouch
- **Price**: ₹0 (TBD)
- **Status**: Description available, product images pending
- **Description**: A clever pencil pouch that also works as a standing pencil organiser. With multiple compartments for pencils, crayons, and stationery, it keeps study time neat and organised. Spacious, functional, and fun for kids.

## Product Features

Our products are characterized by:

- **Premium Quality Materials**: Only the finest materials used in construction
- **Durability**: Built to last with reinforced stitching and quality hardware
- **Functional Design**: Thoughtfully designed with practical compartments and organization
- **Aesthetic Appeal**: Modern, stylish designs that complement any lifestyle
- **Versatility**: Suitable for multiple uses and occasions

## Product Information Structure

Each product includes:

- **Name**: Clear, descriptive product name
- **Category**: Classification (Pouch/Bag/etc.)
- **Description**: Detailed information about the product
- **Base Price**: Pricing in INR (Indian Rupees)
- **Features**: Key highlights and specifications
- **Images**: Multiple product images showcasing different angles
- **Stock Status**: Availability information
- **Active Status**: Whether the product is currently available for purchase

## Customer Benefits

### Free Pan India Delivery
- No minimum order value
- Fast and reliable shipping across India
- Track your order in real-time

### Secure Payment
- Safe and encrypted transactions
- Multiple payment options via Razorpay
- Your data is protected

### Easy Returns
- Hassle-free return policy
- Customer satisfaction guaranteed
- Shop with confidence

## Product Management

Products are managed through:
- Unique product IDs (UUID)
- SEO-friendly slugs for URLs
- Multiple image support with primary image designation
- Category-based organization
- Stock tracking (overall or variant-based)
- Active/inactive status for inventory control

## Target Audience

Storee caters to:
- **Parents and Families**: Looking for organized solutions for kids' essentials
- **Travel Enthusiasts**: Need practical organizers for family trips and vacations
- **Students**: Require functional storage for school and study materials
- **Organized Individuals**: Value quality and craftsmanship in everyday storage solutions
- **Gift Shoppers**: Seeking practical and thoughtful presents for kids and families

Our products are especially designed for:
- Kids' daily routines and travel needs
- Family travel and organization
- School and study essentials
- Keeping children's belongings neat and accessible

## Quality Standards

All products at Storee meet:
- Premium material standards
- Quality control checks
- Durability testing
- Customer satisfaction criteria

---

## Product Image Directory Map

Quick reference for product image locations:

```
/Users/kshitij.nawandar/code/storee/frontend/public/images/products/
├── accessories_kit/          (5 images: accessories_kit_1.jpg to 5.jpg)
├── dental_pouch/             (5 images: 1.png to 5.png)
├── foldable_travel_kit/      (5 images: foldable_travel_kit_1.jpg to 5.jpg)
├── multipurpose_pouch/       (10 images: 1-5.jpg, 6-10.png)
├── on_the_go_foldable_pouch/ (1 image: on_the_go_foldable_pouch_1.jpg)
├── pack_a_week_kit/          (5 images: pack_a_week_kit_1.jpg to 5.jpg)
├── packing_cubes/            (5 images: packing_cubes_1.jpg to 5.jpg)
├── shoe_pouch/               (5 images: shoe_pouch_1.jpg to 5.jpg)
└── toiletry_kit/             (5 images: toiletry_kit_1.jpg to 5.jpg)
```

**Total Image Assets**: 46 images across 9 product folders

**Web Path Format**: `/images/products/{product_folder}/{filename}`

**Example**: `/images/products/accessories_kit/accessories_kit_1.jpg`

---

*Last Updated: March 7, 2026*
